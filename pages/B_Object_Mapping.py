...
if len(hosts_options):
    ...
    for i, tab in enumerate(tabs):
        with tab:
            ...
            panels = st.session_state['export_configs']["hosts"][host]["panels"]
            st.write(panels)

            st.multiselect('选择需要转换的类型',types_options,key=host+"_type_key")
            # st.write(st.session_state[host+"_type_key"])

            df_controler = load_sheet(st.session_state["uploaded_file"], '控制器')
            df_controler = df_controler[df_controler['名称'].isin(panels)]
            ...
            if "控制器节点" in st.session_state[host+"_type_key"]:
                st.header("控制器节点")
                st.number_input('Insert base address', value=10000,key=host+"_controller_numberinput_key")
                # st.write(st.session_state[host+"_controller_numberinput_key"])

                df_controler["Modbus"] = list(range(st.session_state[host+"_controller_numberinput_key"], st.session_state[host+"_controller_numberinput_key"] + controler_rows * increment, increment))
                tmp= host+"_controller_dataeditor_key"+"_changed"
                if st.session_state.get(tmp):
                    for key in st.session_state[tmp]:
                        df_controler.loc[key, "Modbus"] = st.session_state[tmp][key]["Modbus"]
                
                controller_result = st.data_editor(df_controler, use_container_width=True, key=host+"_controller_dataeditor_key", disabled=[0, 1, 2, 3, 4, 5, 6],on_change=onChange,args=[host+"_controller_dataeditor_key"])
                pd_list_result.append(controller_result)
            else:
                controller_result = pd.DataFrame()
                st.session_state[host+"_controller_dataeditor_key"+"_changed"] = {}


            if "P2设备" in st.session_state[host+"_type_key"]:
                st.header("P2设备")
                st.number_input('Insert base address', value=20000,key=host+"_P2_numberinput_key")
                # st.write(st.session_state[host+"_P2_numberinput_key"])

                df_p2["Modbus"] = list(range(st.session_state[host+"_P2_numberinput_key"], st.session_state[host+"_P2_numberinput_key"] + p2_rows * increment, increment))
                
                tmp= host+"_p2_dataeditor_key"+"_changed"
                if st.session_state.get(tmp):
                    for key in st.session_state[tmp]:
                        df_p2.loc[key, "Modbus"] = st.session_state[tmp][key]["Modbus"]
                
                p2_result = st.data_editor(df_p2, use_container_width=True,key=host+"_p2_dataeditor_key",disabled=[0, 1, 2, 3, 4, 5, 6],on_change=onChange,args=[host+"_p2_dataeditor_key"])
                pd_list_result.append(p2_result)
            else:
                p2_result = pd.DataFrame()
                st.session_state[host+"_p2_dataeditor_key"+"_changed"] = {}

            if "联动盘" in st.session_state[host+"_type_key"]:
                st.header("联动盘")
                st.number_input('Insert base address', value=30000,key=host+"_linkage_numberinput_key")
                # st.write(st.session_state[host+"_linkage_numberinput_key"])
                df_linkage["Modbus"] = list(range(st.session_state[host+"_linkage_numberinput_key"], st.session_state[host+"_linkage_numberinput_key"] + linkage_rows * increment, increment))
                
                tmp= host+"_linkage_dataeditor_key"+"_changed"
                if st.session_state.get(tmp):
                    for key in st.session_state[tmp]:
                        df_linkage.loc[key, "Modbus"] = st.session_state[tmp][key]["Modbus"]
                
                linkage_result = st.data_editor(df_linkage, use_container_width=True,key=host+"_linkage_dataeditor_key",disabled=[0, 1, 2, 3, 4, 5, 6],on_change=onChange,args=[host+"_linkage_dataeditor_key"])
                pd_list_result.append(linkage_result)
            else:
                linkage_result = pd.DataFrame()
                st.session_state[host+"_linkage_dataeditor_key"+"_changed"] = {}
            
            ################################################
            st.write("Modbus 设置冲突:")
            for key in st.session_state[host+"_controller_dataeditor_key"+"_changed"]:
                val = st.session_state[host+"_controller_dataeditor_key"+"_changed"][key]["Modbus"]
                validateAll(key,val,'controller_result')
            for key in st.session_state[host+"_p2_dataeditor_key"+"_changed"]:
                val = st.session_state[host+"_p2_dataeditor_key"+"_changed"][key]["Modbus"]
                validateAll(key,val,'p2_result')
            for key in st.session_state[host+"_linkage_dataeditor_key"+"_changed"]:
                val = st.session_state[host+"_linkage_dataeditor_key"+"_changed"][key]["Modbus"]
            ...
            if len(pd_list_result) > 0:
                ...
                st.download_button(
                    label="导出CSV文件",
                    data=csv_result,
                    file_name=st.session_state["uploaded_file"].split(".")[0]+'_'+host+'.csv',
                    mime='text/csv',
                )
